...
describe.concurrent("Interpol keyframes", () => {
  it("should interpolate through multiple keyframes", async () => {
    const values: number[] = []
    await new Interpol({
      duration: 100,
      x: [0, 100, 50, 200],
    ...
    // Values should have gone through the intermediate steps
  ...
  it("should start and end properly with multiple keyframe values", async () => {
    await new Interpol({
      duration: 100,
      x: [0, 100, 32, 20, 201, -100, 200, 400, 201, 10000, -24949, 0],
      onStart: ({ x }) => {
        expect(x).toBe(0)
      },
      onComplete: ({ x }) => {
  ...
  it("should reverse through keyframes", async () => {
    ...
    expect(itp2.props.x.value).toBe(0)
  })

  it("should support keyframes with negative values", async () => {
    const itp = new Interpol({
      duration: 200,
      paused: true,
      x: [-100, 100, -50],
